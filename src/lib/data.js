export const siteConfig = {
  name: "Abhishek",
  fullName: "ABHISHEK BOSE",
  heroName: "Abhishek",
  title: "Full Stack Developer",
  tagline:
    "I'm a Full Stack Developer, passionate about solving real-world problems through clean and efficient code.",
  resumeUrl: "/files/Abishek_Bose_Full_Stack_Developer.pdf",
  email: "abishekbose1355@gmail.com",
  socials: {
    linkedin: "https://www.linkedin.com/in/abishekbose5",
    github: "https://github.com/Abishek-Bose",
  },
};

export const services = [
  {
    number: "01",
    title: "Strategic Development",
    description:
      "Align technical decisions with business objectives to deliver maximum impact.",
  },
  {
    number: "02",
    title: "Scalable Architecture",
    description:
      "Design systems that grow with your success and handle increasing demands.",
  },
  {
    number: "03",
    title: "Quality Assurance",
    description:
      "Deliver code that works reliably, performs efficiently, and stands the test of time.",
  },
  {
    number: "04",
    title: "Reliable Delivery",
    description:
      "Establish standards that ensure maintainable, scalable, and production-ready code.",
  },
];

export const projects = [
  {
    title: "Bizotic Technologies",
    description:
      "Built a full-stack digital transformation platform for an IT services company, featuring modern UI, service showcases, and client consultation booking.",
    animation: "/images/web1.json",
    url: "#",
  },
  {
    title: "Demo & Co. Advocates",
    description:
      "Designed and developed a professional law firm website with multi-jurisdiction support across Kolkata and Delhi, practice area pages, and team profiles.",
    animation: "/images/web3.json",
    url: "#",
  },
  {
    title: "Oxford Stone Clinic",
    description:
      "Created a healthcare platform for Dr. Demo Bose's urology practice, featuring appointment booking, robotic surgery information, and patient resources.",
    animation: "/images/web6.json",
    url: "#",
  },
  {
    title: "Bizotic Supervision",
    description:
      "Developed a comprehensive business registration and compliance portal offering startup incorporation, tax filing, and government registration services.",
    animation: "/images/web7.json",
    url: "#",
  },
];

export const marqueeText =
  "GREAT PRODUCTS ARE MADE WHEN STRATEGY + PRECISION WORKS TOGETHER";
